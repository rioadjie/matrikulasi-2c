...
$id = $_GET['id'];
$data = mysqli_query($koneksi, "SELECT * FROM pendaftaran_ujian_kp where id='$id'");
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <div class="card card-info card-outline">
                        ...
                        <div class="card-body">
                            <form method="POST" action="proses_update_daftarujiankp.php" enctype="multipart/form-data">
                                <input type="hidden" name="Id" value="<?php echo $d['Id'];?>">
                                <div class="form-group">
                                    Upload Laporan (*Hanya menerima file PDF)
                                        <input type="file" name="Laporan_KP" id="Laporan_KP" class="form-control">
                                </div>
                                <div class="form-group">
                                    <label for="">Pilih Jadwal Ujian KP</label>
                                    <input type="datetime-local" id="Jadwal_ujian" name="Jadwal_ujian" class="form-control" value="<?php echo $d['Jadwal_ujian'];?>">
                                </div>
                                <?php } ?>
                                <div class="form-group">
                                    <label for="">ID Pendaftaran KP : </label>
                                    <select class="form-select" arial-label="Pendaftaran_KP_Id" name="Pendaftaran_KP_Id">
                                        <option selected>Pilih ID</option>
                                        <?php
                                        $sql=mysqli_query($koneksi, "SELECT * FROM pendaftaran_kp");
                                        while ($data=mysqli_fetch_array($sql)) { 
                                        ?>
                                        <option value="<?=$data['Id']?>"><?=$data['Id']?> - <?=$data['Tempatkp']?></option>

                                        <?php } ?>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label for="">ACC Ujian : </label>
                                    <select class="form-select" arial-label="ACC_ujian_Id" name="ACC_ujian_Id">
                                        <option selected>Pilih ID</option>
                                        <?php
                                        $sql=mysqli_query($koneksi, "SELECT * FROM acc_ujian");
                                        while ($data=mysqli_fetch_array($sql)) { 
                                        ?>
                                        <option value="<?=$data['Id']?>"><?=$data['Id']?> - <?=$data['Dosen_Penguji']?></option>
